...
from group_and_analyze_dct import group_and_analyze, degroup_and_decode
...
for n in range(1, 1024, 16):
    ...
    coeff_list, lbl_y = group_and_analyze(CY, Idx, 1, noret)
        
    #df = pd.DataFrame(data=coeff_list)
    #df.to_csv('Variable_BlockSize_DCT_Coeffs.csv')
    
    
    ###################### Decoding the Y Plane ##################################
    CYrec = degroup_and_decode(coeff_list, lbl_y)
...
df.to_csv('No_coeffs_vs_msepsnr_dct.csv')